...
class CustomSharedPreferens(private val flag: Int) {
    fun getValueSP(context: Context): String {
        return if (flag == 0) {
            context.getSharedPreferences(PREF_NAME, PRIVATE_MODE).getString(PREF_NAME, BotJob.TAG_MENU)
                .toString()
        } else {
            context.getSharedPreferences(PREF_NAME1, PRIVATE_MODE1).getString(PREF_NAME1, "0")
                .toString()
        }
    }
